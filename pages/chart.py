...
notify_trend_follow = st.sidebar.checkbox("🚀 TREND_FOLLOW で通知",  value=True)
...
fast_ma_len  = 5  if interval_label in ["週足", "月足"] else 9   # 順張り用超短期MA

df["MA_fast"]  = df["Close"].rolling(fast_ma_len).mean()
...
# 表示期間フィルタリング
# ===============================
if interval_label in ["1分足", "3分足", "5分足"]:
...
# ===============================
#
# TREND_FOLLOW パラメータ設計思想：
#   分足  : GCが頻発しやすいため出来高閾値を低め・RSI範囲を広めに設定
#           → 乗り遅れを防ぐ
#   日足以上: だましを減らすためRSI範囲を絞り・出来高閾値を高めに設定
#           → 確度重視
#
if interval_label in ["1分足", "5分足"]:
    TF = dict(
        ...
        tf_rsi_lo=48, tf_rsi_hi=70, tf_vol_ratio=1.05,
    )
    ...
    TF = dict(
        ...
        tf_rsi_lo=50, tf_rsi_hi=70, tf_vol_ratio=1.08,
    )
else:  # 日足 / 週足 / 月足
    TF = dict(
        ...
        tf_rsi_lo=52, tf_rsi_hi=68, tf_vol_ratio=1.15,
    )
...
def diagnose_psychology(idx, full_df, high_20_series):
    ...
    prev_hist    = full_df.iloc[prev_pos]["MACD_hist"]
    prev_ma_fast = full_df.iloc[prev_pos]["MA_fast"]
    prev_ma_s    = full_df.iloc[prev_pos]["MA_short"]
    ...
    # ① 超短期MAが短期MAを「今まさに」下から上抜け（GC初回のみ）
    gc_just_crossed = (prev_ma_fast < prev_ma_s) and (ma_fast > ma_s)
    # ② MACDヒストがプラスかつ拡大中
    macd_rising = (hist > 0) and (hist > prev_hist)
    # ③ RSIが足種別の「上昇モメンタム・過熱前」レンジ
    rsi_ok = TF["tf_rsi_lo"] <= rsi <= TF["tf_rsi_hi"]
    # ④ 出来高が足種別の閾値超（動意あり）
    vol_ok = vol > vol_avg * TF["tf_vol_ratio"]

    if gc_just_crossed and macd_rising and rsi_ok and vol_ok:
        return "TREND_FOLLOW", "#FFD700"
...
action_map = {
    ...
    "PLAN_ENTRY":   ("💎 積極買い（逆張り）",  "嵐が去った後の反転ポイント。"),
    "TREND_FOLLOW": ("🚀 順張りエントリー",    "GC確認・モメンタム乗り。過熱前に乗る。"),
    "FEAR":         ("✊ 継続保有",            "同値撤退の誘惑。ここはホールド。"),
    "NEUTRAL":      ("☕ 様子見",              "指標は中央圏。シグナルを待機。"),
}
...
notify_target = (status in ["PLAN_ENTRY", "TREND_FOLLOW", "TAKE_PROFIT", "TREND_BREAK"])
status_just_changed = (
...
should_notify = (
    ...
    (
        ...
        (status == "TREND_FOLLOW" and notify_trend_follow) or
...
notify_messages = {
    "PLAN_ENTRY":   {"title": "💎 ローリスクエントリー！",  "body": f"{company_name}({code}) PLAN_ENTRY シグナル！反転ポイントの可能性あり。"},
    "TREND_FOLLOW": {"title": "🚀 順張りエントリー！",      "body": f"{company_name}({code}) TREND_FOLLOW シグナル！GC＋MACD上向き確認。"},
    "TAKE_PROFIT":  {"title": "🔴 利確タイミング！",        "body": f"{company_name}({code}) TAKE_PROFIT シグナル！天井圏でMACDが失速中。"},
    "TREND_BREAK":  {"title": "🟠 トレンド崩壊警告！",      "body": f"{company_name}({code}) TREND_BREAK シグナル！上昇シナリオ崩壊の可能性。"},
}

# シグナル別サウンドパターン（音程で聞き分けられるよう設計）
sound_patterns = {
    "PLAN_ENTRY":   ([880, 1100, 1320], [0, 0.15, 0.30]),   # 上昇音（明るい）
    "TREND_FOLLOW": ([660, 880, 1100],  [0, 0.12, 0.24]),   # 中音3音（力強い）
    "TAKE_PROFIT":  ([1320, 880, 440],  [0, 0.15, 0.30]),   # 下降音（警戒）
    "TREND_BREAK":  ([440, 330],        [0, 0.20]),          # 低音2音（危険）
}

if should_notify:
    notif  = notify_messages[status]
    freqs, delays = sound_patterns.get(status, ([880], [0]))
    ...
    if notify_sound:
        tones_js = "\n".join([
            ...
            (function(){{
                ...
                gain.gain.setValueAtTime(0.4, ctx.currentTime+{d});
                gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime+{d}+0.3);
                osc.start(ctx.currentTime+{d}); osc.stop(ctx.currentTime+{d}+0.35);
            }})();"""
...
if should_notify:
    banner_colors = {
        "PLAN_ENTRY":   ("#00C781", "💎 PLAN_ENTRY シグナル発生！逆張り・反転ポイントです。"),
        "TREND_FOLLOW": ("#FFD700", "🚀 TREND_FOLLOW シグナル発生！順張りエントリーのタイミングです。"),
...
with col_action:
    font_col = "black" if status == "TREND_FOLLOW" else "white"
    st.markdown(
        f"<p style='margin-bottom:0;font-size:14px;color:#808495;'>⚖️ 売買アドバイス</p>"
        f"<span style='color:{font_col};background-color:{color};padding:3px 12px;"
...
fig.add_trace(go.Scatter(
    x=x_labels, y=df_display["MA_fast"],
    line=dict(color="cyan", width=1.0, dash="dot"),
    name=f"{fast_ma_len}MA(順張り)"
), row=1, col=1)
fig.add_trace(go.Scatter(
...
if not plan_entries.empty:
    plan_x = (plan_entries.index.strftime("%m/%d %H:%M") if is_intraday
               else plan_entries.index.strftime("%Y/%m/%d"))
    fig.add_trace(go.Scatter(
...
# ---- TREND_FOLLOW マーカー（金色★） ----
trend_follows = df_display[df_display["psych_status"] == "TREND_FOLLOW"]
if not trend_follows.empty:
    tf_x = (trend_follows.index.strftime("%m/%d %H:%M") if is_intraday
             else trend_follows.index.strftime("%Y/%m/%d"))
    fig.add_trace(go.Scatter(
        x=tf_x, y=trend_follows["Low"] * 0.996,
        mode="markers",
        marker=dict(color="#FFD700", size=13, symbol="star",
                    line=dict(color="white", width=1)),
        name="🚀 TREND_FOLLOW（順張り）"
    ), row=1, col=1)

# ---- TAKE_PROFIT マーカー（赤▼） ----
take_profits = df_display[df_display["psych_status"] == "TAKE_PROFIT"]
if not take_profits.empty:
    tp_x = (take_profits.index.strftime("%m/%d %H:%M") if is_intraday
             else take_profits.index.strftime("%Y/%m/%d"))
    fig.add_trace(go.Scatter(
...
if not trend_breaks.empty:
    tb_x = (trend_breaks.index.strftime("%m/%d %H:%M") if is_intraday
             else trend_breaks.index.strftime("%Y/%m/%d"))
    fig.add_trace(go.Scatter(
...
fig.add_hline(y=68, line_dash="dot",  line_color="#FFD700", line_width=1.0, row=2, col=1)
fig.add_hline(y=50, line_dash="dot",  line_color="#FFD700", line_width=1.0, row=2, col=1)
...
| マーカー | 状態 | 条件 |
|---|---|---|
| 赤▼ | **TAKE_PROFIT** | RSI>75＋BB超え＋MACD失速＋出来高増 |
...
| マーカー | 状態 | 種別 | 条件 |
|---|---|---|---|
| 緑〇 | **PLAN_ENTRY** | 逆張り | 直近15本にPANIC発生＋MA上回る＋MACDヒスト正値上向き＋RSI 35〜60 |
| 金★ | **TREND_FOLLOW** | 順張り | 超短期MAのGC（瞬間）＋MACDヒスト正値上向き＋RSI足種別レンジ＋出来高増 |
""")
        st.markdown("""
**TREND_FOLLOW の足種別パラメータ：**

| 足種 | 超短期MA | RSIレンジ | 出来高閾値 |
|---|---|---|---|
| 1分・5分足 | 9MA | 48〜70 | 平均×1.05倍 |
| 3分足 | 9MA | 50〜70 | 平均×1.08倍 |
| 日足・週足・月足 | 5MA | 52〜68 | 平均×1.15倍 |

> RSIパネルの**金色点線（50・68）**が目安ラインです。
""")
...
**サウンドはシグナル別に音程が異なります（聞き分け可能）：**
- 💎 PLAN_ENTRY：上昇音3音（明るい）
- 🚀 TREND_FOLLOW：中音3音（力強い）
- 🔴 TAKE_PROFIT：下降音3音（警戒）
- 🟠 TREND_BREAK：低音2音（危険）
